...
public static class TestHelpers
{
    ...
    {
        if (actualValue == null || expectedValue == null) return string.Empty;
        ...
        for (var i = 0; i < maxLen; i++)
            if (diffStyle != DiffStyle.Minimal || i >= minLen || actualValue[i] != expectedValue[i])
                output.AppendLine(
                    $"{(i < minLen && actualValue[i] == expectedValue[i] ? " " : "*")} {i,-3} {(i < expectedValue.Length ? ((int)expectedValue[i]).ToString() : ""),-4} {(i < expectedValue.Length ? expectedValue[i].ToSafeString() : ""),-3}  {(i < actualValue.Length ? ((int)actualValue[i]).ToString() : ""),-4} {(i < actualValue.Length ? actualValue[i].ToSafeString() : ""),-3}" // character safe string
                );
        output.AppendLine();

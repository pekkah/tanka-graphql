...
public class SubscriptionFacts
{
    ...
    public async Task Simple_Scalar()
    {
        ...
        var schema = await new ExecutableSchemaBuilder()
                ...
                new()
            {
                {"count: Int!", b => b.Run((ctx, cancellationToken) =>
                {
                    ...
                    async IAsyncEnumerable<object> RandomStream([EnumeratorCancellation] CancellationToken unsubscribe)
                    {
                        int i = 0;
        ...
        /* When */
        var stream = new GraphQL.Experimental.Executor(schema)
            .SubscribeAsync(new GraphQLRequest()
            {
                Document = query

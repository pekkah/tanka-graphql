...
public class CompleteValueResult : IResolverResult
{
    ...
    public CompleteValueResult(object? value)
    {
        ...
        IsTypeOf = (_, _) =>
            throw new InvalidOperationException(
                "Abstract type value was resolved without actual type or isTypeOf being provided.");
    }
    ...
    {
        return CompleteValueAsync(Value, context.Field.Type, context.Path, context);
    }
        ...
        IResolverContext context)
    {
        if (value is IResolverResult)
            throw new CompleteValueException(
                $"Cannot complete value for field '{Printer.Print(context.Field)}'. Resolving {nameof(IResolverResult)} value is not supported.",
                path,
                context.Selection);
